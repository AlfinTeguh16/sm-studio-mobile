...
export default function NotificationsScreen() {
  ...
  const fetchPage = useCallback(async (p = 1) => {
    ...
    const lastPage = response?.last_page ?? (list.length < 20 ? 1 : p);
    return { list, lastPage: Number(lastPage) || p };
  }, []);
  ...
  const clearRead = async () => {
    ...
    const respondInvite = async (notif: Notif, action: "accept" | "decline") => {
      ...
      try {
        // optimistic: set busy state on that item by marking is_read true
        setItems(prev => prev.map(i => i.id === notif.id ? { ...i, is_read: true } : i));

        // call backend respond endpoint
        await api.bookings.respondToInvite(bookingId, action);

        // success: mark notification read and remove it
