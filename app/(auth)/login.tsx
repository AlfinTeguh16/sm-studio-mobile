...
export default function LoginScreen() {
  ...
  // --- bottom sheet register (tetap seperti versi sebelumnya) ---
  const [sheetVisible, setSheetVisible] = useState(false);
  const slide = useRef(new Animated.Value(0)).current;
  const openSheet = () => { setSheetVisible(true); requestAnimationFrame(() =>
    Animated.timing(slide, { toValue: 1, duration: 280, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start()
  );};
  const closeSheet = () => Animated.timing(slide, { toValue: 0, duration: 220, easing: Easing.in(Easing.cubic), useNativeDriver: true })
  ...
  const onSubmit = async () => {
    try {
      ...
      if (role === "mua") {
        router.replace("/(mua)");     // menuju app/(mua)/index.tsx
      } else {
        router.replace("/(user)");    // menuju app/(user)/index.tsx
      }
    } catch (e: any) {
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <View style={styles.container}>
          ...
          <View style={{ marginBottom: 15 }}>
            ...
            <TextField label="" value={password} onChangeText={setPassword} />
          </View>
      ...
      <Modal visible={sheetVisible} transparent animationType="none" onRequestClose={closeSheet}>
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY }] }]}>
          ...
          <TouchableOpacity
            ...
            onPress={() => { closeSheet(); router.push("/(auth)/register?as=pengguna"); }}
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => { closeSheet(); router.push("/(auth)/register?as=mua"); }}
          >
...
const styles = StyleSheet.create({
  ...
  sheet: { position: "absolute", left: 0, right: 0, bottom: 0, backgroundColor: "#fff",
           paddingHorizontal: 20, paddingTop: 10, paddingBottom: 24, borderTopLeftRadius: 16, borderTopRightRadius: 16, gap: 10 },
